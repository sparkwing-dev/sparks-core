...
func TestServiceStable_CompletedSingleDeployment(t *testing.T) {
	js := `{"runningCount":3,"desiredCount":3,"deployments":[{"status":"PRIMARY","rolloutState":"COMPLETED"}]}`
	stable, err := serviceStable([]byte(js))
	if err != nil {
		t.Fatalf("serviceStable error: %v", err)
	}
	if !stable {
		t.Fatal("expected a completed single-deployment service to be stable")
	}
}

func TestServiceStable_MultipleDeploymentsNotStable(t *testing.T) {
	js := `{"runningCount":3,"desiredCount":3,"deployments":[{"status":"PRIMARY","rolloutState":"IN_PROGRESS"},{"status":"ACTIVE","rolloutState":"COMPLETED"}]}`
	stable, err := serviceStable([]byte(js))
	if err != nil {
		t.Fatalf("serviceStable error: %v", err)
	}
	if stable {
		t.Fatal("expected a service mid-rollout to be unstable")
	}
}

func TestServiceStable_CountMismatchNotStable(t *testing.T) {
	js := `{"runningCount":2,"desiredCount":3,"deployments":[{"status":"PRIMARY","rolloutState":"COMPLETED"}]}`
	stable, err := serviceStable([]byte(js))
	if err != nil {
		t.Fatalf("serviceStable error: %v", err)
	}
	if stable {
		t.Fatal("expected running<desired to be unstable")
	}
}

func TestServiceStable_FailedRolloutErrors(t *testing.T) {
	js := `{"runningCount":1,"desiredCount":3,"deployments":[{"status":"PRIMARY","rolloutState":"FAILED"}]}`
	if _, err := serviceStable([]byte(js)); err == nil {
		t.Fatal("expected an error for a FAILED rollout")
	}
}
