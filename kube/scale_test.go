...
func TestScaleArgs(t *testing.T) {
	got := scaleArgs("prod", "deploy/myapp-canary", 3, nil)
...
func TestScaleArgs_ZeroReplicas(t *testing.T) {
	got := scaleArgs("default", "deploy/myapp-canary", 0, nil)
...
func TestScaleArgs_ExtraArgs(t *testing.T) {
	got := scaleArgs("prod", "deploy/myapp-canary", 3, []string{"--current-replicas=2"})
	want := []string{"scale", "deploy/myapp-canary", "--replicas=3", "-n", "prod", "--current-replicas=2"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("scaleArgs = %v, want %v", got, want)
	}
}
...
func TestScale_PerCallDryRunSkipsExec(t *testing.T) {
	clearKubeEnv(t)
	err := Scale(context.Background(), ScaleConfig{
		Deployment: "deploy/myapp-canary",
		Replicas:   0,
	...
	if err != nil {
		t.Fatalf("per-call DryRun Scale should succeed without a cluster, got: %v", err)
	}
}
