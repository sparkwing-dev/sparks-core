...
func TestPyPIArgs_UvUsesPublishURLNotRepository(t *testing.T) {
	_, args := pypiArgs("uv", PyPIPublishConfig{
		Repository: "testpypi",
		PublishURL: "https://test.pypi.org/legacy/",
	})
	joined := strings.Join(args, " ")
	want := "publish --publish-url https://test.pypi.org/legacy/ dist/*"
	if joined != want {
		t.Errorf("pypiArgs = %q, want %q", joined, want)
	}
	if strings.Contains(joined, "testpypi") {
		t.Errorf("uv must not route Repository into --publish-url: %q", joined)
	}
}

func TestNpmAuthHost(t *testing.T) {
	cases := map[string]string{
		"":                                "registry.npmjs.org/",
		"https://registry.npmjs.org/":     "registry.npmjs.org/",
		"https://npm.pkg.github.com":      "npm.pkg.github.com/",
		"https://r.example.com/artifacts": "r.example.com/artifacts/",
	}
	for registry, want := range cases {
		if got := npmAuthHost(registry); got != want {
			t.Errorf("npmAuthHost(%q) = %q, want %q", registry, got, want)
		}
	}
}
