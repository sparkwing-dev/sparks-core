...
- **cleanup:** unchecked `os.Remove` returns are explicit ignores now.
...
- Initial module. Shared release-and-publish helpers behind the
  ...
  - `CrossBuildGo` compiles a GOOS/GOARCH matrix into a dist directory
    ...
    Build config exposes `Trimpath`, `Tags`, `BuildFlags`, and
    `EnableCgo` escape hatches alongside `LDFlags`.
  - `GitHubRelease`, `NpmPublish`, and `PyPIPublish` wrap
    ...
    build helpers always execute for real. `NpmPublish` authenticates a
    real publish by writing a temporary `.npmrc` (via `--userconfig`)
    since npm reads its token from an `_authToken` line, not an
    environment variable. `PyPIPublishConfig` splits the twine
    `Repository` name from the uv `PublishURL` endpoint so each tool gets
