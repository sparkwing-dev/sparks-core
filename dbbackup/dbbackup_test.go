...
func TestMySQLArgs(t *testing.T) {
	...
	dumpArgs, dumpEnv := mysqlDumpArgs(c, "/tmp/d.sql")
	if got, want := strings.Join(dumpArgs, " "), "--host=h --port=3306 --user=u --result-file=/tmp/d.sql d"; got != want {
	...
	line, _ := mysqlRestoreLine(c, "/tmp/d.sql")
