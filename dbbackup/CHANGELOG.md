...
- **deps:** bump the sparkwing SDK to v0.31.0, which fixes silent
  truncation of large command output in `Exec(...).Lines()`. A dump
  listing longer than the pipe buffer could previously come back short.
- **cleanup:** unchecked `os.Remove` and `Close` returns are explicit
  ignores now.
