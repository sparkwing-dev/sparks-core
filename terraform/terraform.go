...
func Plan(ctx context.Context, cfg Config) (PlanResult, error) {
	...
	err := step.Run(ctx, "terraform plan", func(ctx context.Context) error {
		...
		if cfg.Workspace != "" {
			if _, err := sparkwing.Exec(ctx, "terraform", "workspace", "select", cfg.Workspace).Dir(dir).Run(); err != nil {
				return err
			}
		}
...
func planArgs(cfg Config) []string {
	args := []string{"plan", "-input=false", "-no-color", "-out=" + cfg.planFile()}
...
// applyArgs builds the `terraform apply` argv for a saved plan.
func applyArgs(opt ApplyOptions) []string {
	return []string{"apply", "-input=false", "-no-color", opt.PlanFile}
}
