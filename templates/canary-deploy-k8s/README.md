...
| `registry` | no | | Container registry URL to push to (ECR/GAR/GHCR); empty resolves kind or `SPARKWING_REGISTRY` |
...
| `platform` | no | | `docker build --platform` target (e.g. `linux/amd64`); empty builds native |
| `rollout-timeout` | no | `180s` | Timeout for each rollout-status wait (canary and stable promotion) |
...
4. **promote** runs only when the canary probe passed. It points the
   stable Deployment at the same image and waits for its rollout. If that
   rollout wedges (surge capacity, a PodDisruptionBudget, quota, or the
   app behaving differently at the stable replica count), the
   **rollback-stable** handler reverts the stable Deployment to its
   previous revision via `kubectl rollout undo` so nothing is left
   half-rolled, and the canary is left running for investigation.
5. **cleanup** runs only after a successful promotion and tears the canary
   Deployment and Service down. It is a separate node so a teardown hiccup
   never triggers the stable rollback.
...
- Promotion is a plain `kubectl set image` on the stable Deployment,
  guarded by the `rollback-stable` handler that reverts stable on a wedged
  rollout. Swap the `promote` body if you gate promotion behind a manual
  approval or a longer soak.
- If you build on one architecture and run on another (an arm64 laptop
  pushing to an amd64 cluster), set `platform` to the cluster's
  architecture (e.g. `linux/amd64`); otherwise the canary pod crash-loops
  on an image it cannot exec and the failure surfaces as a probe timeout.
...
The runner needs `docker` and `kubectl` on PATH. The rollout itself is
pure `kubectl` on any cluster, but registry authentication differs by
cloud: an ECR push logs in automatically, while a GAR (or GHCR) push
needs docker configured for that registry first. On GCP run `gcloud auth
configure-docker <region>-docker.pkg.dev` (or wire the credential helper)
before the first run, or the build node's `docker push` fails.
