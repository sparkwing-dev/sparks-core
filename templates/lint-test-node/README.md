...
```sh
sparkwing pipeline new --name lint-test-node --template lint-test-node

# pnpm project, plain JS (no typecheck)
sparkwing pipeline new --name lint-test-node --template lint-test-node \
  --param install-cmd="pnpm install --frozen-lockfile" \
  --param typecheck-cmd=""
```
...
| `node-version` | no | `20` | Informational banner only; `npm ci` ignores .nvmrc, so provide this version via your runner image or a version manager (nvm/volta/fnm) |
...
- The install node is the one network-bound step (it hits the registry),
  so it renders with a bounded retry and timeout. Adjust the
  `Retry(...)` / `Timeout(...)` calls in `Plan` if your registry is
  slower or flakier than the defaults.
