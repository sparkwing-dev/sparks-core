...
| `min-coverage` | no | `80` | Minimum total line coverage as a plain number 0-100 (no % sign); the gate fails below this. Emitted as a numeric literal into the generated code |

## Scaffold

```sh
sparkwing pipeline new --name coverage-gate --template coverage-gated-test
```
