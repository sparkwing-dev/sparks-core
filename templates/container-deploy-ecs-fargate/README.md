...
| `container-name` | yes | | Container within the task definition whose image is swapped |
| `health-url` | yes | | URL the post-deploy probe checks for a 2xx |
| `region` | no | `us-west-2` | AWS region of the ECS cluster |
...
- A `Dockerfile` at the repo root (the `build` node runs `docker build`
  against it).
- An existing ECS service on `cluster` whose task-definition `task-family`
  ...
  revision, swaps the image on `container-name`, and registers a new
...
2. re-registers it as a fresh revision with `container-name`'s image set
...
- The probe accepts any 2xx. Tighten it with `.ExpectStatus(200)` or
  ...
  structured output.
- The image is pushed to `ecr` and referenced by its content-addressed
  production tag. Adjust `imageRef` if your task definition pins a
