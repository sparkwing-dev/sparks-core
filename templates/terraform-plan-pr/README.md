...
- `var-file` is passed to plan as `-var-file`. Empty omits it. Point it at a
  per-environment tfvars file (`prod.tfvars`) to plan that environment. It
  accepts a single file; to layer a shared base plus a per-environment
  overlay, edit the generated `plan` node to append more entries to
  `cfg.VarFiles`.
...
- State-backend access (credentials, backend config) comes from the ambient
  ...
  infrastructure. The backend must be fully specified in the `.tf` backend
  block: this scaffold does not pass `-backend-config` to `terraform init`, so
  per-environment partial backend config (a bucket or key that varies by
  environment) has to live in the backend block or be added by editing the
  generated `plan` node to set `cfg.Backend`.
...
- The webhook payload is the Slack incoming-webhook shape. Point
  ...
  A non-empty `notify-webhook` is baked into the scaffolded pipeline source in
  cleartext and committed with it. A Slack incoming-webhook URL is a bearer
  credential, so for a public or shared repo prefer leaving it empty and
  editing the generated `report` node to read the URL from a sparkwing secret.
