...
and `MATRIX_VERSION` exported, then executes `test-cmd` inside it. This
needs a Docker daemon and pulls `<repo>:<version>` for each version, so
it does not run without local Docker; the host mode (empty
...
| `setup-cmd` | no | (empty) | optional per-cell setup before `test-cmd`, sees `MATRIX_VERSION` |
...
- `MATRIX_VERSION` is a plain environment export. The default `test-cmd`
  ignores it (runs the whole suite unchanged in every cell); wire it
  into `setup-cmd` (asdf / gvm / nvm) or into `container-image-repo` to
  actually vary the toolchain per cell.
