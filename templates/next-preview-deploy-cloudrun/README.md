...
2. Builds the Docker image from `dockerfile` in `build-context` (defaults
   `./Dockerfile`) with a `<branch-slug>-<timestamp>` tag, so each push of a
   branch produces a fresh image and a new preview revision.
...
4. Deploys the pushed image to Cloud Run with
   ...
   tagged preview URL at `health-path`, and prints it.

## Scaffold

```sh
sparkwing pipeline new --template next-preview-deploy-cloudrun \
  --param image=web \
  --param registry=us-west1-docker.pkg.dev/my-project/my-repo \
  --param service=my-web \
...
| `port` | no | `3000` | Container port the Next server listens on (empty leaves Cloud Run's default) |
| `health-path` | no | `/` | Path appended to the preview URL for the post-deploy probe |
| `allow-unauthenticated` | no | `true` | Public preview when `true`; `false` (or `0`/`no`/empty) deploys a private, authenticated preview |
| `env` | no | | Runtime env vars as `KEY=VAL,KEY=VAL`, passed via `--set-env-vars` |
...
## Public or private previews

By default (`allow-unauthenticated=true`) the preview revision is publicly
reachable, so the probe can hit it with a bare HTTP GET. Set
`allow-unauthenticated=false` for a private preview behind IAM/IAP -- useful
when the branch ships unreleased features you do not want exposed on the open
internet. A private preview answers an unauthenticated probe with 401/403,
which `probe.HTTP` treats as indeterminate and fails the run, so add an
identity token to the probe in `verify` (e.g. a `HeaderFunc` supplying
`Authorization: Bearer <token>`) when you make previews private.
...
- The `build` node runs `docker build -f <dockerfile> <build-context>`.
  Point `dockerfile` at your Next.js `Dockerfile` (build with
  `output: standalone`), set `build-context` if it lives in a monorepo
  subdirectory (e.g. `apps/web`), and adjust `port` to the standalone
  server's port (the default is `3000`).
- The image tag is `<branch-slug>-<UTC-timestamp>`, computed once in `build`
  ...
  prefer.
- The probe accepts any 2xx at the tagged preview URL plus `health-path`.
  Tighten it in the `verify` method with `.ExpectStatus(200)` or
  `.ExpectJSON("status", "ok")` if your health endpoint returns structured
  output.
