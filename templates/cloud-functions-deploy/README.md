...
| `allow-unauthenticated` | no | `` | HTTP triggers: non-empty makes it public; empty (default) keeps it private and the probe authenticates with an identity token |
...
- `http` deploys an HTTP function (`--trigger-http`), and adds
  ...
  this kind resolves a URL and runs the health probe. The default is empty,
  so the function is private: set `allow-unauthenticated` to any non-empty
  value (e.g. `true`) to expose it publicly.
...
- For an HTTP trigger the deploy then resolves the function URL with
  ...
  `<function-url><health-path>` for any 2xx. When the function is private
  (`allow-unauthenticated` empty) the probe attaches a `gcloud
  auth print-identity-token` bearer for the function URL, so the runner
  identity needs the Cloud Run Invoker role on the function. Tighten the
  check in the `verify` method with `.ExpectStatus(200)` or
  `.ExpectJSON("status", "ok")` if your health endpoint returns structured
  output, and widen the readiness window with `.Retry(n)` / `.Interval(d)`
  for a slow-starting function.
