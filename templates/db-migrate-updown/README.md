...
`down-steps` must be a non-negative integer. It renders straight into the
generated Go as the rollback count, so a non-numeric value produces a
scaffold that will not compile.
...
credentials. The generated body invokes the binary by name; to use a
vendored or version-pinned one, set `Binary` on the `migrate.Config` it
builds.

If a migration fails partway, golang-migrate marks the schema version
dirty and refuses every later up/down run until the flag is cleared. To
recover, reconcile the schema by hand, then run `migrate force <version>`
against the DSN. The `migrate` block exposes `migrate.Force` if you want
to build that recovery path into the pipeline itself.
