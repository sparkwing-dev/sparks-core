...
| `watch-paths` | no | `*.go,go.mod,go.sum` | Comma list of git pathspecs (matched recursively); a change under any runs the job |
...
The defaults gate a Go test suite: `*.go,go.mod,go.sum` covers every
tracked Go file at any depth (git pathspecs match `/`), and `work-cmd`
runs `go test ./...`. Retarget both for another stack or another subtree
-- see [After rendering](#after-rendering).
