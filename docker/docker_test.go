...
func TestBuildCacheRef_Registry(t *testing.T) {
	for _, backend := range []string{"ecr", "gar"} {
...
func TestBuildxPublish_RequiresImageAndRegistry(t *testing.T) {
	if err := BuildxPublish(context.Background(), BuildxConfig{Registry: "ghcr.io/org"}); err == nil {
		t.Fatal("BuildxPublish without Image = nil, want error")
	}
	if err := BuildxPublish(context.Background(), BuildxConfig{Image: "myapp"}); err == nil {
